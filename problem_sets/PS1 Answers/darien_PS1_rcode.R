...
#The confidence interval is (94.1, 102.7). There is a 90% probability that a confidence interval would contain the true average IQ of students in the school with repeated sampling.
#For example, if we gathered 100 samples, we would expect 90 of the CIs calculated to contain the population mean (student IQ in this school) and we would expect 10 of the CIs to not include the population mean.
...
y <- c(105, 69, 86, 100, 82, 111, 104, 110, 87, 108, 87, 90, 94, 113, 112, 98, 80, 97, 95, 111, 114, 89, 95, 126, 98)
SE <- (sample_sd / sqrt(length(y)))
SE
t <- ((sample_mean - 100)/ SE)
1 - pt(abs(t), df=24, lower.tail = F)
